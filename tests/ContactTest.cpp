...
TEST_CASE_FIXTURE(Reporting, "reporting: support and impact forces account for momentum") {
    ...
    SUBCASE("reporting: impact impulse matches momentum with and without restitution") {
    ...
    SUBCASE("reporting: sliding impulse opposes slip and accounts for momentum") {
        Floor();
        const auto body = Box();
        solver.Advance(world, {}, 120);
    ...
    SUBCASE("reporting: force application points account for angular momentum") {
        Floor();
...
TEST_CASE_FIXTURE(Reporting, "reporting: patch geometry survives manifold reduction") {
    SUBCASE("reporting: octagonal patch retains area before reduction to four points") {
        Floor();
    ...
    SUBCASE("reporting: an edge has zero area and a nonzero extent") {
        Floor();
        const float angle = 0.4f;
...
TEST_CASE_FIXTURE(Reporting, "reporting: snapshots retain lifetime and collider attribution") {
    SUBCASE("reporting: queued samples survive separation and host-side removal") {
        Floor();
        const auto body = Box();
    ...
    SUBCASE("reporting: compound leaf manifolds retain collider attribution") {
    ...
    SUBCASE("reporting: mesh triangle keys remain on the correct side after canonicalization") {
...
TEST_CASE_FIXTURE(Reporting, "reporting: enabling measurement preserves motion") {
...
TEST_CASE_FIXTURE(Reporting, "collision: local frames and surface sidedness reach both contact sides") {
    SUBCASE("planes: a double-sided surface supports either side") {
    ...
    SUBCASE("compounds: the root collider frame reaches both sides of collision") {
    ...
    SUBCASE("meshes: double-sided surfaces keep one winding and preserve seams") {
