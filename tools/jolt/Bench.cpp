...
int main(int argc, char **argv) {
    ...
    if (workers < 0 || velocity_steps <= 0 || position_steps <= 0 || warmup < 0 || timed <= 0) return 1;
    ...
    std::printf("Jolt, workers=%d, dt=1/60, velocity iterations=%d, position iterations=%d, sleep off, damping zero, slop=0.0005, warmup=%d, timed=%d\n", workers, velocity_steps, position_steps, warmup, timed);
    bool ran = false;
    for (auto scene : {Scene{"floor", 1, 1, 1}, Scene{"stack20", 1, 1, 20}, Scene{"lattice600", 10, 10, 6}, Scene{"lattice6000", 25, 40, 6}, Scene{"lattice96000", 100, 160, 6}, Scene{"lattice192000", 200, 160, 6}}) {
        ...
        RefConst<Shape> box = new BoxShape(Vec3::sReplicate(0.5f), 0);
        std::vector<BodyID> ids;
        std::vector<std::array<float, 2>> origins;
        for (int x = 0; x < scene.across; ++x)
            for (int z = 0; z < scene.deep; ++z)
                for (int y = 0; y < scene.high; ++y) {
                    BodyCreationSettings bs(box, RVec3(1.5f * x, 0.5f + 1.02f * y, 1.5f * z), Quat::sIdentity(), EMotionType::Dynamic, 1);
                    bs.mFriction = 0.5f;
                    bs.mRestitution = 0;
                    ...
                    bs.mMassPropertiesOverride.mMass = 1000;
                    const auto body = bi.CreateBody(bs);
                    if (!body) return 2;
                    ids.push_back(body->GetID());
                    origins.push_back({1.5f * x, 1.5f * z});
                }
        auto added = ids;
        ...
        for (int step = 0; step < warmup + timed; ++step) {
            ...
            auto result = world.Update(1.f / 60, 1, &alloc, jobs.get());
            ...
            for (size_t i = 0; i < ids.size(); ++i) {
                ...
                quality.Observe({float(p.GetX()), float(p.GetY()), float(p.GetZ()), q.GetX(), q.GetY(), q.GetZ(), q.GetW(), v.GetX(), v.GetY(), v.GetZ(), w.GetX(), w.GetY(), w.GetZ()}, origins[i], int(i % scene.high));
            }
        }
