...
struct PileChurn {
    ...
    static std::map<Name, Row> Read(const World &world) {
        ...
        EachActive(world, [&](const Contact &contact) {
            ...
            rows.emplace(Name{contact.BodyA, contact.BodyB, contact.Feature, contact.SubShape, contact.Children}, Row{WorldPoint(pose, contact.AnchorA), -contact.Lambda[0]});
        });
        return rows;
    }
...
void Raft(std::span<char *> args) {
    ...
    for (uint32_t step = 0; step < steps; ++step) {
        ...
        std::println("{:4} churn={:4} engaged={:4} renamed={:4} flicker={:4} asleep={:3}/{} refused={:4} |v|max={:8.5f} top y={:7.4f} normal={:9.1f} spread={:8.2f}", step, churn.All, churn.Engaged, churn.Renamed, churn.Flicker, Asleep(world, boxes, settings), boxes.size(), refused, fastest, float(world.Poses[boxes.back()].Position.y), churn.Load, churn.Spread);
    }
...
void Coins(std::span<char *> args) {
    ...
    const float twist = ArgF(args, 5, 0.5f);
...
// Apply a lateral force at half the static friction limit after different settling durations.

void Shear(std::span<char *> args) {
    const uint32_t hold = Arg(args, 2, 600), boxes = std::max(2u, Arg(args, 3, 4));
    ...
    for (const uint32_t settled : {60u, 600u, 1800u}) {
        ...
        for (uint32_t step = 0; step < hold; ++step) {
            ...
            std::println("{:5} drift {:10.3e} m  vx {:10.3e} m/s  |  rows {} stuck {}  friction penalty {:9.4g}..{:9.4g} N/m  "
                         ...
                         step, drift, float(world.Velocities[top].Linear.x), rows.Count, rows.Stuck, rows.LowPenalty, rows.HighPenalty, simd::length(rows.Tangential), rows.Cone, float(world.Poses[stack.front()].Position.x) - floor_from);
        }
        const float drift = float(world.Poses[top].Position.x) - from;
    ...
    for (const auto &outcome : outcomes)
        std::println("settled {:5} steps: drift {:10.3e} m, creep over the last second {:10.3e} m/s, friction penalty {:9.4g} N/m", outcome.Settled, outcome.Drift, outcome.Creep, outcome.Penalty);
}
...
void Motion(std::span<char *> args) {
    ...
    constexpr float Speed = 4, Spin = 5;
...
void Compound(std::span<char *> args) {
    ...
    std::println("  inertia ({:9.5f} {:9.5f} {:9.5f}) against ({:9.5f} {:9.5f} {:9.5f}) kg m^2", 1 / solid.InvInertiaLocal.x, 1 / solid.InvInertiaLocal.y, 1 / solid.InvInertiaLocal.z, 1 / built.InvInertiaLocal.x, 1 / built.InvInertiaLocal.y, 1 / built.InvInertiaLocal.z);
    std::println("  the pieces put the body frame at ({:8.5f} {:8.5f} {:8.5f}), turned {:9.2e} rad - the middle of the two, which is where the one box already was", float(halved_frame.Position.x), float(halved_frame.Position.y), float(halved_frame.Position.z), simd::length(RotationVector(halved_frame.Orientation)));
    ...
    for (uint32_t step = 0; step < steps; ++step) {
        ...
        if (!report(step)) continue;
        std::println("{:4}  one y={:8.5f} v={:7.4f} a/mu g={:6.3f}  |  halves y={:8.5f} v={:7.4f} a/mu g={:6.3f}  |  apart {:9.2e} m", step, float(whole.Poses[one_box].Position.y), previous_whole, slowed_whole / coulomb, float(split.Poses[two_halves].Position.y), previous_split, slowed_split / coulomb, apart);
    }
    ...
    std::println("a table of a slab and four legs, its body frame at ({:8.5f} {:8.5f} {:8.5f}) in the frame it was authored in", float(table_frame.Position.x), float(table_frame.Position.y), float(table_frame.Position.z));
    for (uint32_t step = 0; step < steps; ++step) {
        solver.Step(table_world, settings);
        if (!report(step)) continue;
        const Pose pose = table_world.Poses[table];
        const float3 top = WorldPoint(pose, top_in_body);
        std::println("{:4}  y={:8.5f} of {:8.5f} authored  tilt={:9.2e}  contacts={:2} refused={}  |  top at ({:8.5f} {:8.5f} {:8.5f}), authored ({:8.5f} {:8.5f} {:8.5f})", step, float(pose.Position.y), float(table_frame.Position.y), simd::length(RotationVector(QuatMul(pose.Orientation, QuatConjugate(table_frame.Orientation)))), ActiveContacts(table_world, table), table_world.ContactRefusals[table], float(top.x), float(top.y), float(top.z), float(top_authored.x), float(top_authored.y), float(top_authored.z));
    }
    ...
    const uint32_t welded = separate.WeldStatic();
    ...
    for (uint32_t step = 0; step < steps; ++step) {
        ...
        std::println("{:4}  compound x={:8.5f} y={:8.5f} vx={:7.4f} rows={:2}  |  two bodies x={:8.5f} y={:8.5f} vx={:7.4f}  |  one box x={:8.5f} y={:8.5f} vx={:7.4f}", step, float(jointed.Poses[over_seam].Position.x), seam_y, seam_v, ActiveContacts(jointed, over_seam), float(separate.Poses[over_two].Position.x), two_y, two_v, float(seamless.Poses[over_none].Position.x), none_y, none_v);
    }
    ...
    for (uint32_t step = 0; step < steps; ++step) {
        ...
        std::println("{:4}  y={:8.5f} of {:8.5f}  tilt={:9.2e}  contacts={:2} of {}  refused={}", step, float(pose.Position.y), double(Cube), simd::length(RotationVector(pose.Orientation)), ActiveContacts(eight, row), ContactsPerBody, eight.ContactRefusals[row]);
    }
}
...
void Join(std::span<char *> args) {
    ...
    std::println("a box slid at {:g} m/s into the join of two separate static boxes, swept by how far the far one stands proud, against a margin of {:.1e} m", speed, double(margin));
    ...
    for (uint32_t step = 0; step < steps; ++step) {
        ...
        for (uint32_t i = 0; i < worlds.size(); ++i)
            std::print("  |  {:.2f}xm x={:8.5f} y={:8.5f} vx={:7.4f}", double(steps_high[i] / margin), float(worlds[i].Poses[sliders[i]].Position.x), float(worlds[i].Poses[sliders[i]].Position.y), last[i]);
        std::println("");
    }
    std::println("  the control travelled {:8.5f} m and kept {:7.4f} m/s", float(seamless.Poses[alone].Position.x) - SlideFrom, last_alone);
    for (uint32_t i = 0; i < worlds.size(); ++i)
        std::println("  step {:5.2f} x margin: travelled {:8.5f} m, kept {:7.4f} m/s, worst kick {:.3e} m, worst extra loss in one step {:.3e} m/s against Coulomb's {:.3e}, resting on {} rows", double(steps_high[i] / margin), float(worlds[i].Poses[sliders[i]].Position.x) - SlideFrom, last[i], kick[i], worst_loss[i], coulomb, ActiveContacts(worlds[i], sliders[i]));
}
...
FaceOwners Owners(const World &world) {
    FaceOwners owners;
    EachActive(world, [&](const Contact &contact) {
        if (contact.BodyA == 0 || contact.BodyB == 0) return;
...
void Platform(std::span<char *> args) {
    ...
    std::println("a kinematic body: no inverse mass, and the host writes its pose and a velocity to match\n"
                 "  every step. {} steps at {:g} m/s, which is {:.5f} m a step",
                 steps, speed, speed * dt);

    {
        ...
        for (uint32_t step = 0; step < steps; ++step) {
            ...
            std::println("  {:4} slab x={:8.4f} box x={:8.4f} slipped {:9.6f} box vx={:8.5f} of {:g} asleep={}", step, float(world.Poses[slab].Position.x), float(world.Poses[box].Position.x), float(world.Poses[box].Position.x - world.Poses[slab].Position.x) - held, float(world.Velocities[box].Linear.x), speed, world.Quiet[box] >= settings.SleepSteps);
        }
    }
...

    {
        ...
        std::println("a stack of {} asleep on a slab that then starts moving: {}/{} asleep when it does", stack.size(), Asleep(world, stack, settings), stack.size());
        ...
        for (uint32_t step = 0; step < steps; ++step) {
            ...
            for (const auto body : stack)
                std::print("  x={:8.4f} slipped {:9.6f} y={:7.4f}", float(world.Poses[body].Position.x), float(world.Poses[body].Position.x - world.Poses[slab].Position.x) - held, float(world.Poses[body].Position.y));
            std::println("");
        }
    }
}
...
void Wheel(std::span<char *> args) {
    ...
    std::println("a paddle wheel as an open mesh on a hinge, {} balls dropped onto it over {} steps.\n"
                 "  the bare hub is the same wheel with its paddles taken off, which is the control:",
                 balls, steps);
    for (const uint32_t vanes : {paddles, 0u}) {
        ...
        world.Poses[floor].Position.y = -2;
        ...
        for (uint32_t step = 0; step < steps; ++step) {
            ...
            std::println("    {:4} turned {:9.4f} rad  rate {:8.4f} rad/s  balls {:2}  rows {:3}  off its axle {:.6f} m", step, turned, float(world.Velocities[wheel].Angular.z), dropped, ActiveContacts(world), simd::length(world.Poses[wheel].Position));
        }
        std::println("    -> {:9.4f} rad, {:6.2f} turns, fastest {:8.4f} rad/s at step {}", turned, turned / (2 * std::numbers::pi_v<float>), fastest, spun_at);
    }

    {
        ...
        AddGround(world);
        world.AddBody({.Pose = At(float3{6, 0.75f, 0}), .Shape = world.AddShape({.HalfExtents = {2, 0.25f, 2}, .Kind = ShapeBox}), .Density = 0, .Friction = Friction});
        ...
        std::println("a {:g} m mesh cube dropped from 3 m onto a plane at y = 0 and a box with its top at y = 1:\n"
                     "  over the plane it is at y={:9.4f}, expected {:g}\n"
                     "  over the box   it is at y={:9.4f}, expected {:g}",
                     2 * MeshHalf, float(world.Poses[on_plane].Position.y), MeshHalf, float(world.Poses[on_box].Position.y), 1 + MeshHalf);
    }
}
...
void Perch(std::span<char *> args) {
    ...
    for (uint32_t step = 0; step < steps; ++step) {
        ...
        for (uint32_t side = 0; side < 2; ++side) {
            ...
            std::println("  {:7} {:4} churn={:4} asleep={}/2  box |v|={:8.5f} y={:7.4f} tilt={:8.5f}  mesh |v|={:8.5f} |w|={:8.5f} y={:7.4f} tilt={:8.5f}", side == 0 ? "moving" : "scenery", step, churn[side].Take(), Asleep(world, bodies, settings), simd::length(world.Velocities[box[side]].Linear), float(world.Poses[box[side]].Position.y), tilt(box[side]), simd::length(world.Velocities[mesh[side]].Linear), simd::length(world.Velocities[mesh[side]].Angular), float(world.Poses[mesh[side]].Position.y), tilt(mesh[side]));
            std::println("            box on mesh: {:2} rows{}\n            mesh on slab: {:2} rows{}", ActiveContacts(world, box[side]), PairRows(world, box[side], mesh[side]), ActiveContacts(world, slab[side]), PairRows(world, slab[side], mesh[side]));
        }
    }
}
...
// Sample a plate with a convex parabolic underside to test contacts on individual facets.
...
void Rocker(std::span<char *> args) {
    ...
    for (uint32_t step = 0; step < steps; ++step) {
        ...
        std::println("  {:4} plate rows={:2} |v|={:8.5f} |w|={:8.5f} tilt={:8.5f} wander={:8.5f} fn=[{:8.1f},{:8.1f}] churn={:3} asleep={}", step, ActiveContacts(world, plate), p.Fastest, p.Turned, Tilt(world, plate), p.Wander, p.Low, p.High, churner.Take(), Asleep(world, std::span{&plate, 1}, settings));
        std::println("       ball  rows={:2} |v|={:8.5f} |w|={:8.5f} tilt={:8.5f} wander={:8.5f} fn=[{:8.1f},{:8.1f}]           asleep={}", ActiveContacts(world, ball), b.Fastest, b.Turned, Tilt(world, ball), b.Wander, b.Low, b.High, Asleep(world, std::span{&ball, 1}, settings));
    }
}
...
void Teleport(std::span<char *> args) {
    ...
    std::println("  bottom of the stack:  woken y={:7.4f} x={:7.4f} asleep {}/{}  |  left alone y={:7.4f} x={:7.4f} asleep {}/{}", woken.StackY, woken.StackX, woken.StackAsleep, boxes, left.StackY, left.StackX, left.StackAsleep, boxes);
    ...
    std::println("  the stack falls {:.4f} m when woken and {:.4f} m when not, and the box it landed on moves {:.4f} m against {:.4f} m", SlabTop + Half - woken.StackY, SlabTop + Half - left.StackY, woken.PushedX - Apart, left.PushedX - Apart);
}

void Bullet(std::span<char *> args) {
    const float thickness = ArgF(args, 2, 0.02f);
    constexpr float Size = 0.05f, From = -1, Reach = 2;
...
// Use gravity projected into the swing plane for the pendulum energy reference.
...
FramedHinge HangOnFrame(World &world, float4 frame, float4 pivot_turn, float3 gravity, float distance, float release) {
    ...
    const float3 down = simd::normalize(in_plane);
    ...
    const auto shape = world.AddShape(UnitBox);
    const auto pivot = world.AddBody({.Pose = At(float3{0, 0, 0}, pivot_turn)});
...
void JointFrame(std::span<char *> args) {
    ...
    FramedHinge hinges[]{HangOnFrame(worlds[0], frame, turns[0], settings.Gravity, Distance, release), HangOnFrame(worlds[1], frame, turns[1], settings.Gravity, Distance, release)};

    std::println("a hinge free about ({:.3f} {:.3f} {:.3f}), which is no axis of either body, on two pivots turned differently.", float(hinges[0].Axis.x), float(hinges[0].Axis.y), float(hinges[0].Axis.z));
    std::println("  released {:.0f} degrees off the bottom, energy says it reaches {:.4f} rad/s there. Backwards Euler dissipates,", release / Degrees, hinges[0].Peak);
    ...
    for (uint32_t step = 0; step < steps; ++step) {
        ...
        std::println("{:4} |w| {:8.4f} of {:7.4f} ({:6.1f}%)  out of plane {:9.6f}  pivots apart {:9.6f}  arm ({:6.3f} {:6.3f} {:6.3f})", step, peak, hinges[0].Peak, 100 * peak / hinges[0].Peak, out_of_plane, apart, float(at.x), float(at.y), float(at.z));
    }
    std::println("peak {:.4f} rad/s is {:.1f}% of the closed form, it left the plane by {:.6f} m, and the two pivots never diverged past {:.6f} m", peak, 100 * peak / hinges[0].Peak, out_of_plane, apart);
    ...
    for (const float offset : {0.f, 0.01f}) {
        ...
        std::println("  ends {:.3f} m apart at rest: {:9.6f} m apart after {} steps, under 0.1 mm at step {}, fastest they ever closed {:8.5f} m/s", offset, gap, steps, when, worst_closing);
    }
    ...
    const float speed = wanted / (float(steps) * spinning.DeltaTime);
    {
        ...
        for (uint32_t step = 0; step < steps; ++step) {
            ...
            std::println("  {:4} turned {:9.4f} rad ({:7.3f} turns)  |w| {:8.4f}  off the axle {:9.6f} rad/s", step, turned, turned / (2 * std::numbers::pi_v<float>), simd::length(world.Velocities[wheel.Body].Angular), off_axis);
        }
        std::println("  -> {:.4f} rad of the {:.4f} asked for ({:.2f}%), and it never turned more than {:.6f} rad/s off the axle", world.Joints[wheel.Joint].Twist, wanted, 100 * world.Joints[wheel.Joint].Twist / wanted, off_axis);
    }

    {
        ...
        for (uint32_t step = 0; step < steps; ++step) {
            ...
            if (!report(step)) continue;
            const float turned = world.Joints[wheel.Joint].Twist;
            std::println("  {:4} at {:9.4f} rad ({:7.2f} degrees)  |w| {:8.5f}", step, turned, turned / Degrees, simd::length(world.Velocities[wheel.Body].Angular));
        }
...
void Slider(std::span<char *> args) {
    ...
    const auto run = [&](const char *what, JointDesc joint, float3 at, const StepSettings &use, float3 move_to, uint32_t move_at) {
        ...
        for (uint32_t step = 0; step < steps; ++step) {
            ...
            std::println("  {:4} at ({:8.5f} {:8.5f} {:8.5f})  v ({:8.5f} {:8.5f} {:8.5f})", step, float(at_now.x), float(at_now.y), float(at_now.z), float(v.x), float(v.y), float(v.z));
        }
        return std::pair{world.Poses[slider.Box].Position, world.Velocities[slider.Box].Linear};
    };

    const auto [dropped, dropped_v] = run("a slider free along y between +/- 0.5, dropped from its zero onto the lower stop:", {.Angular = {AxisLocked, AxisLocked, AxisLocked}, .Linear = {AxisLocked, AxisLimited, AxisLocked}, .LinearLimitLow = {0, -Stop, 0}, .LinearLimitHigh = {0, Stop, 0}}, float3{0, 0, 0}, settings, float3{}, ~0u);
    std::println("  -> rests at y {:.6f} against the stop at {:g}, {:.6f} m under it, going {:.6f} m/s", float(dropped.y), -Stop, -Stop - float(dropped.y), simd::length(dropped_v));
    ...
    const auto boxed = run("free in a box, all three axes limited to +/- 0.25, gravity pointing into one corner:",
                           ...
                           .first;
    std::println("  -> rests at ({:.6f} {:.6f} {:.6f}), against the corner at ({:g} {:g} {:g})", float(boxed.x), float(boxed.y), float(boxed.z), -Reach, -Reach, -Reach);
    ...
    std::println("  -> {:.6f} m/s, of the {:g} asked for. F/m is {:.4f} m/s^2, so it took {:.3f} s to arrive", float(driven.x), Speed, Force / mass, Speed * mass / Force);
...
void Brake(std::span<char *> args) {
    ...
    const float damping = ArgF(args, 4, 166.667f);

    {
        ...
        std::println("a wheel of inertia {:.4f} spun to {:g} rad/s against a k = 0, c = {:g} brake. Time constant I/c is {:.4f} s", wheel.Inertia, Spin, damping, wheel.Inertia / damping);
...

    {
        ...
        for (const bool damped : {false, true}) {
            ...
            world.AddJoint({.BodyA = box, .BodyB = anchor, .At = {0, 0, 0}, .LinearStiffness = {INFINITY, Stiffness, INFINITY}, .LinearDamping = {0, damped ? critical : 0, 0}});
            ...
            std::println("  c = {:9.1f} ({:12}): rests at {:9.6f} against mg/k of {:9.6f}, dipped {:9.6f} m past that, and came back up to {:9.6f}", damped ? critical : 0, damped ? "critical" : "no damper", float(world.Poses[box].Position.y), rest, std::max(0.f, rest - lowest), highest);
        }
    }
}
...
void Pinned(std::span<char *> args) {
    ...
    std::println("a mass of zero is an infinite mass, not an absent one: nothing translates the body and\n"
                 "  its inertia still turns it. {} steps at {:g} Hz",
                 steps, 1 / dt);

    {
        ...
        const auto pinned = world.AddBody({.Pose = At(float3{0, 3, 0}), .Shape = world.AddShape(UnitBox), .Mass = {{.Mass = 0, .Inertia = {inertia, inertia, inertia}}}});
        const Pose was = world.Poses[pinned];
        for (uint32_t step = 0; step < steps; ++step) solver.Step(world, settings);
        std::println("  hanging in gravity for {} steps: moved {:.9f} m, and its inverse inertia is {:g}", steps, float(simd::distance(world.Poses[pinned].Position, was.Position)), float(world.Masses[pinned].InvInertiaLocal.x));

        world.Velocities[pinned] = {.Angular = {0, 1.5f, 0}};
        for (uint32_t step = 0; step < 120; ++step) solver.Step(world, settings);
        std::println("  spun at 1.500000 rad/s and left alone for 120: reads {:.6f}, and has moved {:.9f} m", float(world.Velocities[pinned].Angular.y), float(simd::distance(world.Poses[pinned].Position, was.Position)));
    }
    ...
    for (const float friction : {0.f, 0.5f}) {
        for (const bool turns : {true, false}) {
            ...
            std::println("  mu {:3.1f} inertia {:5.1f}: J {:8.4f} N s, spin {:9.6f} rad/s against r x J / I of {:9.6f}, moved {:.9f} m", friction, turns ? Inertia : 0, impulse, float(world.Velocities[pinned].Angular.z), turns ? -Offset * impulse / Inertia : 0, float(simd::distance(world.Poses[pinned].Position, was.Position)));
        }
    }

    {
        ...
        const auto floor = AddGround(world);
        world.Poses[floor].Position.y = -2;
        const auto wheel = world.AddBody({.Shape = shape, .Mass = {{.Mass = 0, .Inertia = {0, 0, 20 * WheelRadius * WheelRadius / 2}}}, .Friction = Friction, .AngularDamping = 0.6f});
        ...
        for (uint32_t step = 0; step < 600; ++step) {
            ...
            std::println("  {:4} turned {:9.4f} rad  rate {:8.4f} rad/s  off its place {:.9f} m  off its axis {:.9f} rad", step, turned, float(world.Velocities[wheel].Angular.z), wandered, tilted);
        }
        std::println("    ->{:9.4f} rad, {:6.2f} turns, never further than {:.9f} m from where it started", turned, turned / (2 * std::numbers::pi_v<float>), wandered);
    }

    {
        ...
        const auto slab = world.AddBody({.Pose = At(float3{0, -0.25f, 0}), .Shape = world.AddShape({.HalfExtents = {3, 0.25f, 3}, .Kind = ShapeBox}), .Mass = {{.Mass = 0, .Inertia = {200, 200, 200}}}, .Friction = Friction});
        const float inertia = cube_inertia(BoxMass, 1);
        const auto box = world.AddBody({.Pose = At(float3{0, Half + 0.2f, 0}), .Shape = world.AddShape(UnitBox), .Mass = {{.Mass = BoxMass, .Inertia = {inertia, inertia, inertia}}}, .Friction = Friction});
        ...
        std::println("a box of {:g} kg resting on a pinned slab: rests at {:.6f}, slab sank {:.9f} m and turned {:.9f} rad/s,\n"
                     ...
                     BoxMass, float(world.Poses[box].Position.y), float(simd::distance(world.Poses[slab].Position, was.Position)), float(simd::length(world.Velocities[slab].Angular)), rows, rows > 0 ? penalty / float(rows) : 0, rows > 0 ? penalty / float(rows) / inertial : 0);
    }

    {
        ...
        const auto pinned = world.AddBody({.Velocity = {.Angular = {0, 0, 2}}, .Shape = world.AddShape(UnitBox), .Mass = {{.Mass = 0, .Inertia = {inertia, inertia, inertia}}}, .Friction = 0, .AngularDamping = 4});
        for (uint32_t step = 0; step < steps; ++step) solver.Step(world, free_space);
        std::println("a pinned box spun at 2 rad/s into a damper: after {} steps it reads {:.6f} rad/s, asleep={}", steps, float(world.Velocities[pinned].Angular.z), world.Quiet[pinned] >= free_space.SleepSteps);
        world.AddBody({.Pose = At(float3{-1.2f, 0.25f, 0}), .Velocity = {.Linear = {4, 0, 0}}, .Shape = world.AddShape({.Radius = 0.25f, .Kind = ShapeSphere}), .Mass = {{.Mass = 30, .Inertia = {1, 1, 1}}}, .Friction = 0});
        for (uint32_t step = 0; step < 30; ++step) solver.Step(world, free_space);
        std::println("  and a ball into its face 30 steps later: {:.6f} rad/s, asleep={}", float(world.Velocities[pinned].Angular.z), world.Quiet[pinned] >= free_space.SleepSteps);
    }
}
} // namespace
...
int main(int argc, char **argv) try {
    ...
    const std::pair<std::string_view, void (*)(std::span<char *>)> scenes[]{
        {"stack", Stack}, {"slam", Slam}, {"slide", Slide}, {"shear", Shear}, {"bounce", Drop}, {"seams", Seams}, {"raft", Raft}, {"bullet", Bullet}, {"order", Order}, {"coins", Coins}, {"motion", Motion}, {"offset", Offset}, {"teleport", Teleport}, {"platform", Platform}, {"wheel", Wheel}, {"perch", Perch}, {"rocker", Rocker}, {"frame", JointFrame}, {"slider", Slider}, {"brake", Brake}, {"pinned", Pinned}, {"compound", Compound}, {"join", Join}
    };
