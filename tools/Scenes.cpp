...
ContactTotals Totals(const World &world) {
    ...
    for (uint32_t slot = 0; slot < world.Contacts.Capacity; ++slot) {
        ...
        ++totals.Count;
        // The frame the solve applied these forces in, so what comes back out is what went in.
...
// NOSLEEP=1 stops bodies sleeping, so a scene that would settle goes on being solved. A five-box
// stack falls asleep mid-sway and its velocity then reads zero because sleeping sets it so, which hid
// a ringing stack for a whole session.
void ApplyNoSleep(StepSettings &settings) {
    if (getenv("NOSLEEP")) settings.SleepSteps = ~0u;
}

// And which steps a scene reports on: one in `Period` and the last, or every one under EVERY=1, which
// is coarse sampling hiding the same ringing from the other side.
struct Reporting {
...
void Raft(std::span<char *> args) {
    ...
    for (uint32_t step = 0; step < steps; ++step) {
        ...
        std::println("{:4} churn={:4} asleep={:3}/{} refused={:4} |v|max={:8.5f} top y={:7.4f}", step, churner.Take(), Asleep(world, boxes, settings), boxes.size(), refused, fastest, float(world.Poses[boxes.back()].Position.y));
    }
